...
grep -q "versionName '1.0.10'" app/build.gradle
grep -q 'DB_VERSION = 2' app/src/main/java/pl/paczkahub/data/AppDatabase.java
grep -q 'extractInlineImageBase64' app/src/main/java/pl/paczkahub/mail/MimeDecoder.java
grep -q 'pickupDeadline' app/src/main/java/pl/paczkahub/PackageDetailActivity.java
...
javac -encoding UTF-8 -d /tmp/parser-test \
  ...
  tests/TestParser109.java tests/TestPickup110.java
java -cp /tmp/parser-test TestParser109
java -cp /tmp/parser-test TestPickup110
...
cp "$APK" dist/PaczkaHub-1.0.10.apk
sha256sum dist/PaczkaHub-1.0.10.apk > dist/SHA256SUMS.txt
rm -rf paczkahub/app/build paczkahub/.gradle paczkahub/build
cd paczkahub
zip -qr ../dist/PaczkaHub-1.0.10-kod-zrodlowy.zip . -x '*/build/*' '.gradle/*' '*.apk' '*.aab' 'local.properties'
